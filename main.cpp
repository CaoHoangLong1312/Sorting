...
void insertionSort(int arr[], int n)
{
	...
	for (int i = 1; i < n; i++)
	{
		...
		// Đếm phép so sánh current > 0
        comparisonCount++;
        while (current > 0) {
            // Đếm phép so sánh arr[current - 1] > temp
            comparisonCount++;
            if(arr[current - 1] > temp){
                arr[current] = arr[current - 1];
                current--;
            } else {
                break;
            }
            // Đếm phép so sánh current > 0 ở vòng lặp tiếp theo
            comparisonCount++;
        }
...
int medianOfThree(int a[], int left, int right) {
    ...
    if (++comparisonCount && a[left] > a[mid]) {
        HoanVi(a[left], a[mid]);
    }

    if (++comparisonCount && a[left] > a[right]) {
        HoanVi(a[left], a[right]);
    }

    if (++comparisonCount && a[mid] > a[right]) {
        HoanVi(a[mid], a[right]);
    }
...
void processSorting(const string &algorithm, int arr[], int n, const string &outputParam) {
	const int NUM_RUNS = 1; // if you want to run multiple times, change NUM_RUNS
...
void process2Sorting(const string &algorithm1, const string &algorithm2, int arr1[], int arr2[], int n, const string &outputParam) {	
	...
	const int iterations = 1; // if you want to run multiple times, change iterations 
